...
4. The following XML code contains some syntax errors.
	...
	The children tag needs to be in `"` quotation marks. This is also the case for the title language. English should be in quotation marks. While not a syntax error, there should also be a dot after J. It should read J. K. Rowling.
...
6. The following XML code contains syntax errors.
	``` XML
	<message>The salary for the junior developer < 2000 </message>
	```

	Indicate the error in this XML code and provide the correct XML code.  
	The error is that they have used a `<` in the text content. In an XML document this is a special character that you use for the start of a tag and can not be used in text. We can however use a entity reference like this:
	``` XML
	   <message>The salary for the junior developer %lt; 2000</message>
	```


8. The following XML code contains some syntax errors.
	``` XML
	<film><title>Emancipation<actor>Will Smith
	<year>2022</film></title></actor></year>
	```
	Indicate the error in this XML code and provide the correct XML code.
	It seems like the writer of this code have tried to nest tags, but they are jumbled. Also, the order would be in the wrong way when they don't place the root tag (`<film>`, `</film>`) first and last. 
	``` XML
	<film>
